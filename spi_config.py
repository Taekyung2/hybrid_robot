...
# -----------------------------------------------------------------------------------------------
    ...
    def __init__(self, spi_bus_number = 0, spi_dev_number = 1):
        ...
        self.gyro_divider = 0
        self.acc_divider = 0
        self.calib_data = [0, 0, 0]
        self.magnetometer_ASA = [0, 0,0]
        self.temperature = 0
        self.gyroscope_data = [0, 0, 0]
        self.accelerometer_data = [0, 0, 0]
        self.magnetometer_data = [0, 0, 0]
...
# -----------------------------------------------------------------------------------------------
    ...
    def testConnection(self):
        response = self.ReadReg(self.__MPUEG.WHOAMI)
        if (response == 0x71):
            return True
        else:
            return False
...
# -----------------------------------------------------------------------------------------------
    ...
    def read_acc(self):
        ...
        for i in range(0, 3):
            data = self.byte_to_int(response[i*2:i*2+2])
...
# -----------------------------------------------------------------------------------------------
    ...
    def read_temp(self):
        ...
        temp = self.byte_to_int(response)
...
# -----------------------------------------------------------------------------------------------
   ...
   def read_mag(self):
        ...
        for i in range(0, 3):
            data = self.byte_to_int_le(response[i*2:i*2+2])
            self.magnetometer_data[i] = data * self.magnetometer_ASA[i]
        return (self.magnetometer_data[0], self.magnetometer_data[1], self.magnetometer_data[2])
...
# -----------------------------------------------------------------------------------------------
    ...
    def read_all(self):
        ...
        # Get temperature
        i = 3
        temp = self.byte_to_int(response[i*2:i*2+2])
        self.temperature = (temp/340.0)+36.53
...
# -----------------------------------------------------------------------------------------------
    ...
    def byte_to_int(self, input_buffer):
        byte_array = array.array("B", input_buffer)
        signed_16_bit_int, = struct.unpack(">h", byte_array)
        return signed_16_bit_int


# -----------------------------------------------------------------------------------------------


    def byte_to_int_le(self, input_buffer):
        byte_array = array.array("B", input_buffer)
        signed_16_bit_int, = struct.unpack("<h", byte_array)
        return signed_16_bit_int
...
# -----------------------------------------------------------------------------------------------
    ...
    def getHeading(self):
        (scaled_x,scaled_y,scaled_z) = self.read_mag()
...
# -----------------------------------------------------------------------------------------------
    ...
    def getHeadingString(self):
        degrees = self.getHeading()
	degrees1 = degrees/100
	degrees2 = (degrees/10)%10
	degrees3 = degrees % 10
	return str(int(degrees1)) + str(int(degrees2)) + " " + str(int(degrees3))
